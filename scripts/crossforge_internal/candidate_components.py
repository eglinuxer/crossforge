...
def prepare(source, binding_path, directory, data, builder, oras, cosign, docker_config=None):
    ...
    inputs = capture(source, final, execution, bindings)
    ready = {"schema_version": 1, "kind": "crossforge-candidate-component-binding", "source_commit": commit,
...
def check(source, binding_path, directory, sha256, builder, docker_config=None):
    ...
    exact_fields(ready, ("schema_version", "kind", "source_commit", "source_binding_sha256", "execution", "bindings",
                        "graph_sha256", "inputs", "selection"), "candidate component binding")
    require(type(ready["schema_version"]) is int and ready["schema_version"] == 1 and
    ...
    require(ready["source_commit"] == commit and ready["source_binding_sha256"] == content_sha256(binding) and
            ready["execution"] == execution, "candidate source, source bundle or build environment changed")
    ...
    component_inputs.require_match(ready["inputs"], capture(source, resolved, execution, ready["bindings"]))
    return ready
