#!/usr/bin/env python3
"""Render cache-scoped release component identities from release.json.
...
def vcpkg_contract_policy_materials():
    return sorted(
        [
        {
            "path": "/@implementation/vcpkg-contract/%s"
            % "/".join(str(part) for part in path),
            "value": copy.deepcopy(value),
        }
        for path, value in leaf_items(VCPKG_CONTRACT_POLICY)
        ],
...
def _render_expected_components(release, implemented_rows):
    ...
    add(
        ...
        "qualification",
        explicit_materials=vcpkg_contract_policy_materials(),
    )
    add(
        "zstd/host-build",
        "build",
