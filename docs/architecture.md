...
仅重跑失败的 native/signing job 时，通过成功上游的 artifact ID 取得原产物，保留发布、原生执行和签名各自的 run attempt。签名 artifact 中的 `candidate-recovery.json` 绑定完整 candidate manifest 摘要、probe/report 字节摘要和原 artifact ID/name/attempt；promotion schema 2 将这份来源记录嵌入 `release-promotion.json`，经 GitHub 同 run/source 元数据和既有语义验证后进入持久归档。旧 schema 1 保持所有 artifact 同 attempt 的严格契约。publish job 内部中途失败、跨 candidate run 恢复及实际 GitHub 部分重试验收尚未完成。
