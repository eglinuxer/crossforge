...
本地 OCI 元数据及整行安装树由 BuildKit 以仅含 COPY 的配方导出 tar 流，避免逐文件目录同步停顿。`local_export.py` 限制每次传输为十分钟；完整归档在新的私有目录中解包，先核验全部路径、文件类型和链接，再保留原权限进入既有字节、契约、报告及安装树验收。超时后终止该 Docker 客户端及其 Buildx 子进程，保留未完成归档，使用相同固定 digest 和配方向新的归档与目录重试一次；普通传输错误、损坏归档和解包错误直接失败，第二次超时仍失败。行 CI 在正常中间目录清理前保存两次导出配方及超时记录。该机制不重跑资格，也不把中断的 producer 标记为成功。
