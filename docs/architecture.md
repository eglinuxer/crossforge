...
双 target 的 `install_manifest.txt` 也属于受审证据：x86_64 为 13,649 条、aarch64 为 13,650 条，二者均对应 11,894 个唯一安装路径。允许的重复严格限于 125 个 `.prl` 文件各出现 15 次和五个公共生成头各出现两次；aarch64 只额外允许 NEON 私有头 `qdrawhelper_neon_p.h` 出现两次。任何新增、消失或次数变化都使资格失败。上述 cross-build tier 从不执行 target 代码；锁定 sysroot、干净 Rocky 8.10 和原生 ARM 的 Qt 运行时资格仍是独立的后续门禁。
