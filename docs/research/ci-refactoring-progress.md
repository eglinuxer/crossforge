...
## Docker 验收：原重放批次完成，补齐增量实验的独立审计输入（2026-09-11）

固定 a56bb29 的顺序 batch 已退出 0，包含六行资格、六个独立安装、两种 SDK、vcpkg 和两个源码重放范围。最后 cp39 阶段为 1,240.976 秒，三个 owning stage 共六条 fresh RUN：三条原 CPython 编译、三条原行输入校验，不是六次编译。原图还实际重建了缓存未命中的 AArch64 binutils/GCC；额外开销如实保留。断网 Docker 独立重算原 source plan 并复核六条 RUN 通过。新版 append 的 cp310 独立安装也通过，1,232.190 秒包含共享 builder 等待；原两条 RUN、行 manifest 和未改写的 receipt/producer 复核通过。其余四行与两种 SDK 仍在当前配方 batch 内推进。
...
## 配置测试进程并行补充（2026-09-11，本地已验证，GitHub 待测）

独立 `codex/quick-test-parallelism` 分支在 `ca4d429` 上实现完整 unittest 发现、四个进程和独立源码副本；计时权重只调度模块，不删除测试。新增 16 项真实子进程回归覆盖发现失败、遗漏/重复、fixture 跳过、失败汇总、源码变更、超时和取消。quick 工作流保存每项测试身份、日志及时间；原串行 CLI 保留。

相同固定 Docker、4 CPU/16 GiB 的单次控制实验中，原 1,431 项从含发现的 346.447 秒降到 99.193 秒，两侧测试身份逐项一致、无跳过。随后生产入口完整运行 1,447 项为 104.880 秒、打包 40 项通过，均无跳过；锁定验证、生成物、语法、其余 Bake 图和 actionlint 通过。最终小改动另以 16 项 runner、16 项 planner、实际 Rocky 8 四条 RUN 及原范围原生静态检查复验，没有将分次结果写成一次完整复跑。真实材料比较仅选 `platform-python-check`，GCC/CPython 编译材料不变。首次测试 fixture 和文件模式实验的失败均保留在[验证记录](configuration-test-parallelism-2026-09-11.json)。

这是本地测试调度优化，尚未推送本分支或测得 GitHub 提速；Python 构建矩阵仍为 2。现有 PR #3 的 `ca4d429` 长任务继续运行。全部原 PR 检查通过后再合入 main，然后执行 main 专属组件签名、候选与原生 ARM 验收；六批次剩余门槛不变。
