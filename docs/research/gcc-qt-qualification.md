...
DejaGNU 1.6.1's local `unix_load` executes a produced program directly and does not consume an `exec_shell` board field. Crossforge therefore overrides `unix_load` in each AArch64 board and prefixes every execution with the pinned QEMU, runtime root, CPU, and uname policy while preserving DejaGNU status/output handling. The generic DejaGNU QEMU board is unsuitable because its aarch64 GNU branch forces static linking. [DejaGNU `unix_load` source](https://gnu.googlesource.com/dejagnu/+/refs/heads/origin/config/unix.exp), [DejaGNU board manual](https://www.gnu.org/software/dejagnu/manual/Board-configuration-file.html)
...
Qt therefore follows GCC as a separate test-only supply-chain slice. The source boundary now locks the archive and checksum evidence and validates all 399,185 members, eight required module roots, and top-level licenses offline. Host tools, WebEngine tools, and dual-architecture dependency overlays remain the next prerequisites before any Qt compilation. Qt build outputs never enter the SDK.

Rocky 8.10 AppStream provides Node.js 20 and Python 3.8 module streams, while its packaged html5lib is the pure-Python `python3-html5lib` installed under the platform Python 3.6 path. A pinned-container installation test proved that Python 3.8 imports html5lib, six, and webencodings when and only when that path is supplied explicitly. The host qualification contract therefore records this bridge and must test the imports rather than inheriting a developer `PYTHONPATH`. [Rocky AppStream repository](https://download.rockylinux.org/pub/rocky/8.10/AppStream/x86_64/os/)
