...
本页是六项已确认方案的验收索引，便于评审；详细历史在[实施进度](ci-refactoring-progress.md)。截至 `9f8d305`，实现位于 `codex/component-ci-refactor`，远程 main 仍为 `cf736ea`。本页不构成候选或发布资格证据。
...
| 按组件与下游选择 | 材料闭包、领域失效、未知路径全量兜底和完整结果集合校验已接入 | 一次真实受影响变更的计划与执行对照，以及 GitHub 动态任务验收 |
...
| 资格复用、强制重放与恢复 | 原 producer/receipt/执行记录保留；主行工作流接入签名交接，SDK 最终集成强制执行；raw 与完整 32+6 恢复记录、候选分阶段 checkpoint 和同 run 部分重试契约已有回归 | vcpkg 锁定源码资格与 GCC/CPython 强制源码重放正在推进；真实 GitHub 恢复、候选最终集成与原生 ARM、签名及 digest-only promotion |
...
最新发现与修复：真实 cp39 补丁实验暴露了独立 append 对完整 release 的依赖，已改用本行固定组件，修复后的计划只选 inputs、cp39 和 SDK。[本批记录](python-append-scope-2026-09-11.json)分别保存首次完整回归的旧断言失败和修正后 75 项复验。新配方的六个独立安装与两种 SDK 实际验收正在运行，上表中的 a56bb29 历史 Docker 成功结果不代替这轮验收。
